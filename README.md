...
What holds and what does not - summarized from [`docs/audit/FINAL_FINDINGS.md`](docs/audit/FINAL_FINDINGS.md) and [`docs/audit/ABLATION_FINDINGS.md`](docs/audit/ABLATION_FINDINGS.md):
...
- **Unsupported paper claims - do not cite as published findings:**
  - The crater-disjoint split claim is contaminated: `train.npy` contains ~4,250 patches byte-identical to `shackleton_02` per class. Honest generalization must use the rebuilt per-strip splits (`results/split_integrity/`), where IoU drops by ~1.1 pts.
   - The ablation table in `paper.tex` (IoU 0.44-0.55) used a 12% data subset / 30-epoch / double-sigmoid regime and is **not comparable** to the full-data baseline. Full-data ablations are in [`docs/audit/FINAL_FINDINGS.md` §5](docs/audit/FINAL_FINDINGS.md) (e.g., no-CLAHE -5.1 pts, no-augmentation -6.9 pts, `pos_weight=3.0` is harmful vs. `1.0` on full data).
...
├── docs/
│   ├── REPRODUCIBILITY.md        # dataset provenance, hardware, env, seeds
│   ├── audit/                    # audit and findings (FINAL_FINDINGS, ABLATION_FINDINGS, etc.)
│   └── archive/                  # HANDOVER, report (historical)
├── results/                      # all rerun outputs, checksums, plots (generated, ignored)
...
Detailed tables, plots, error analysis, and the expert-annotation export are in `results/` and documented in `docs/audit/FINAL_FINDINGS.md`.
...
- `docs/audit/FINAL_FINDINGS.md` - complete audit with 8 integrity issues and corrected tables to use if you fork the paper.
- `docs/audit/ABLATION_FINDINGS.md` - legacy 12% ablation details (not comparable to full-data, kept for provenance).
...
@unpublished{lucid_draft_2026,
  ...
  author = {Sankalp H S},
  year   = {2026},
  note   = {Unpublished draft; see docs/audit/FINAL_FINDINGS.md for audit}
}
```
